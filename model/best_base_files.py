...
def get_files(time_window):
    ...
    for base_file in BEST_BASE_FILES:
        try:
            ...
            base_files.append([base_file[0].format(time_window, param_value), base_file[1], base_file[2].format(time_window)])
        except KeyError:
            base_files.append([base_file[0].format(time_window), base_file[1],  base_file[2].format(time_window)])
