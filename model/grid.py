    ...
    for time_group in time_grouped:
        ...
        for row in g_data:
            ...
            if method == 'signal_sum':
                ...
                index_count[index_tup] = max(100 + int(row[2]), index_count[index_tup])
...
def normal_hadamard_grid_method(data, mac, size, std, method):
    results = []
    drone_data = read_drone_data()
    prev_pos = None
    prev_time = None
    ...
    for time_group in time_grouped:
        ...
        if prev_pos is None or (time_group[0] - prev_time) > MAX_SECONDS_DIFF:
            ...
            norm_matrix = create_norm_matrix((prev_pos[1], prev_pos[0]), std_matrix, dimensions)
            prev_pos = get_max_matrix_value(np.multiply(index_count, norm_matrix))
